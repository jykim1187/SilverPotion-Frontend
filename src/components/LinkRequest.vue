  ...
  <v-dialog v-model="showDialog" max-width="500px">
    <v-card class="link-request-card">
      <v-card-title class="link-request-title">
        ...
        <h2>{{ currentStep === 1 ? '피보호자 연결 가능 개수' : '피보호자 연결 요청' }}</h2>
      </v-card-title>
        ...
        <v-card-text class="link-request-content">
          <div class="cash-item-info">
            ...
            <div class="cash-item-description">
              <p>피보호자 연결을 위해서는 힐링포션 1개가 필요합니다.</p>
      ...
      <template v-else>
        <v-card-text class="link-request-content">
          <div class="description">
            <p>피보호자로 연결하고 싶은 분의 휴대폰 번호를 입력해주세요.</p>
...
export default {
  ...
  methods: {
    ...
    async sendRequest() {
      ...
      try {
        const response = await axios.post(`${process.env.VUE_APP_API_BASE_URL}/user-service/silverpotion/carelink/send`, {

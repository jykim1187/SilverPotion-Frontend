<template>
  <v-container>
    ...
    <div class="d-flex align-center mb-4">
      ...
      <span class="text-h6 ml-3">자유글 작성</span>
      <v-spacer></v-spacer>
      <v-btn 
        color="primary" 
        @click="savePost" 
  ...
  methods: {
    async initPost() {
      try {
        ...
        // 임시 게시글 초기화 API 호출
        const requestData = {
          postCategory: 'free', // 백엔드 Enum 형식에 맞춤
        };
    ...
    async savePost() {
      ...
      try {
        ...
        const response = await axios.put(
          `${process.env.VUE_APP_API_BASE_URL}/post-service/silverpotion/post/update/free/${this.postId}`,
          formData,
          {
            headers: {
              'X-User-LoginId': loginId,
        ...
        if (response.data && response.data.result) {
          this.showSnackbar('게시글이 저장되었습니다.', 'success');
        ...
        this.showSnackbar('게시글 저장에 실패했습니다.', 'error');
      } finally {
        this.loading = false;
      }
    },
